<template>
  <div
    ...
    :style="[customClass ? {} : { width: parseInt(width) < 100 ? '100px' : width }]"
    v-click-outside
  >
        ...
        <div class="mzl-select-option-find">
          <ul>
            <li
              ...
              :class="{
                'mzl-select-active': activeIndex == index || selVal == item[labelFiled] || item.selected,
...
<script setup>
import { ref, computed, reactive, onMounted, watch, nextTick, onUpdated } from 'vue';
...
const selectClass = computed(() => {
  return [`mzl-select-${props.size}`, props.disabled ? `mzl-select-${props.size}-disabled` : ''];
});
// select input class

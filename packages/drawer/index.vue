    ...
    <div :class="['mzl-drawer-contain', customClass]" v-if="modelValue" :style="styles">
      <div class="mzl-drawer-contain-box">
        <div
          ...
          :style="{ padding: showClose ? '0 20px 0 45px' : '0 20px 0 20px' }"
          v-if="showHeader"
        >
          ...
          <div class="mzl-drawer-header-btn" v-if="!$slot['header']">
            <atd-button
              size="small"
              :style="{ 'margin-right': confirmShow ? '10px' : '0' }"
              v-if="cancelShow"
              @click="cancelBtn"
              >{{ cancelText }}</atd-button
            >
            <atd-button size="small" type="primary" v-if="confirmShow" @click="confirmBtn">{{
              confirmText
            }}</atd-button>
          </div>
        </div>
...
const styles = computed(() => {
  return [
    props.direction == 'left'
      ? { top: 0, left: 0, width: props.width, height: '100%' }
      : props.direction == 'top'
      ? { top: 0, left: 0, width: '100%', height: props.height }
      : props.direction == 'right'
      ? { top: 0, right: 0, width: props.width, height: '100%' }
      : props.direction == 'bottom'
      ? { bottom: 0, left: 0, width: '100%', height: props.height }
      : {}
  ];
});
...
.fade-pupop-enter-active,
.fade-pupop-leave-active {
  transition: opacity 0.2s ease;
}
.fade-pupop-enter-from,
.fade-pupop-leave-to {
  opacity: 0;
